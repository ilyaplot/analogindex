    ...
    public function actionProduct($brand, $product, $page = null, $link = null, $id = null, $debug=false)
    {
        ...
        if ($page > 0 && $criteria->offset > 0) {
            ...
            $prev_url = Gallery::getUrl(
                            $product->brand_data->link, $product->link, $gallery[0]->image_data->name, $gallery[0]->id, ($page - 1 == 1) ? null : $page - 1);
        ...
        if ($pages->getPageCount() != $page && $pages->getPageCount() > 1) {
            ...
            $last_url = Gallery::getUrl(
                            $product->brand_data->link, $product->link, $last_url->image_data->name, $last_url->id, ($page + 1 == 1) ? 2 : $page + 1);
        ...
        foreach ($gallery as $key => &$item) {
            ...
            if (isset($gallery[$key + 1])) {
                $item->next_url = Gallery::getUrl(
                                $product->brand_data->link, $product->link, $gallery[$key + 1]->image_data->name, $gallery[$key + 1]->id, $page);
            } else {
                $item->next_url = $last_url;
            }

            $item->self_url = Gallery::getUrl(
                            $product->brand_data->link, $product->link, $item->image_data->name, $item->id, $page);
