...
class PHPCrawlerHTTPRequest
{
    ...
    public function addProxy($proxy_host, $proxy_port, $proxy_username = null, $proxy_password = null)
    {
        $proxy = [
        ...
        $this->proxyList[md5(serialize($proxy))] = $proxy;
    ...
    public function setProxy($proxy_host, $proxy_port, $proxy_username = null, $proxy_password = null)
    {
        ...
        $this->addProxy($proxy_host, $proxy_port, $proxy_username, $proxy_password);
    }
    ...
    public function sendRequest()
    {
        ...
        if ($PageInfo->error_code != null) {
             ...
             * 
             */
            unset($this->proxyList[md5(serialize($this->proxy))]);
            if (!empty($this->proxyList)) {
                echo "Unable to connect to proxy '" . $this->proxy["proxy_host"] . "' on port '" . $this->proxy["proxy_port"] . "', trying to get new proxy from list..." . PHP_EOL;
                $this->proxy = reset($this->proxyList);
    ...
    protected function openSocket(&$error_code, &$error_string)
    {
        ...
        // Open socket
        if ($this->proxy != null) {
            $this->socket = @stream_socket_client($this->proxy["proxy_host"] . ":" . $this->proxy["proxy_port"], $error_code, $error_str, $this->socketConnectTimeout, STREAM_CLIENT_CONNECT);
        } else {
